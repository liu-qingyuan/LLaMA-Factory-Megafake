...
该脚本专门用于分析Zero-shot with Decomposition Format (ZS-DF)格式的模型预测结果。
ZS-DF格式包含：
...
脚本会从Final Classification部分提取预测结果，如果没有明确结论则通过关键词匹配判断。
"""
...
def normalize_prediction(pred_text, task_type, task2_subclass=None):
    ...
    # 优先提取 Final Classification 部分
    final_classification_patterns = [
        r'5\.\s*\*\*final classification\*\*[:\s]*(.*?)(?:\.|$)',
        r'\*\*final classification\*\*[:\s]*(.*?)(?:\.|$)',
        r'final classification[:\s]*(.*?)(?:\.|$)',
        r'based on.*analysis.*(?:the )?(?:news )?article is\s*(\w+)',
        r'(?:the )?(?:news )?article is\s*(?:likely\s*)?(\w+)',
        r'(?:classification|conclusion)[:\s]*(\w+)'
    ]
    
    for pattern in final_classification_patterns:
        match = re.search(pattern, pred_clean)
        if match:
            classification_text = match.group(1).strip().lower()
            
            # 检查分类结果
            if any(word in classification_text for word in ['legitimate', 'true', 'real', 'credible']):
                return "legitimate"
            elif any(word in classification_text for word in ['fake', 'false', 'misleading', 'fabricated']):
                return "fake"
    
    # 如果没有找到明确的Final Classification，尝试从整个文本的末尾提取
    # 检查最后100个字符中的关键词
    text_end = pred_clean[-100:]
    
    # 查找明确的结论词汇
    if any(word in text_end for word in ['fake', 'false', 'misleading', 'fabricated']):
        return "fake"
    elif any(word in text_end for word in ['legitimate', 'true', 'real', 'credible']):
        return "legitimate"
    
    # 如果仍然没有找到明确答案，回退到传统的关键词匹配
...
def calculate_binary_classification_metrics(data, task_type, task2_subclass=None):
    ...
    for item in data:
        ...
        # 从label字段的ZS-DF格式中提取真实标签
        label_text = str(item['label']).strip()
        
        # ZS-DF格式的标签以"legitimate"或"fake"结尾
        true_label = None
        if label_text.lower().strip().endswith('legitimate'):
            true_label = 'legitimate'
        elif label_text.lower().strip().endswith('fake'):
            true_label = 'fake'
        else:
            # 如果没有明确的结尾，跳过这个样本
            continue
...
def get_dataset_from_path(file_path):
    ...
    # ZS-DF 数据集识别
    if "zs_df" in filename.lower() or "ZS_DF" in filename:
        if "_glm_" in filename:
            return "ChatGLM ZS-DF Dataset"
        elif "_llama3_" in filename or "_llama_" in filename:
            return "LLaMA ZS-DF Dataset"
        elif "_qwen" in filename.lower():
            return "Qwen ZS-DF Dataset"
        elif "_baichuan" in filename.lower():
            return "Baichuan ZS-DF Dataset"
        elif "_mistral" in filename.lower():
            return "Mistral ZS-DF Dataset"
        else:
            return "ZS-DF Dataset"
...
def find_result_files(base_dir="megafakeTasks"):
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for file in files:
            # 只查找包含zs_df的jsonl文件，且在full目录下
            if file.endswith('.jsonl') and 'zs_df' in file.lower() and 'full' in root:
...
def main():
    parser = argparse.ArgumentParser(description='分析ZS-DF模型预测结果')
    parser.add_argument('--dir', default='megafakeTasks', help='结果文件目录')
    parser.add_argument('--file', help='指定单个文件进行分析')
    parser.add_argument('--output', default='megafakeTasks/results/prediction_analysis_results_ZS_DF.csv', help='CSV输出文件名')
    parser.add_argument('--no-csv', action='store_true', help='不导出CSV文件')
    
    args = parser.parse_args()
    
    print("🚀 开始分析ZS-DF模型预测结果")
