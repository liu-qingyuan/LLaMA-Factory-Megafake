...
该脚本专门用于分析Chain-of-Thought with Self-Consistency (CoT-SC)格式的模型预测结果。
CoT-SC格式包含：
- 5个推理链（Reasoning Chain 1-5）
- 自一致性投票（Self-Consistency Vote）  
- 最终答案（Final Answer）

脚本会优先提取Final Answer，如果没有则从Self-Consistency Vote中提取，
最后通过统计推理链结论来确定预测结果。
"""
...
def normalize_prediction(pred_text, task_type, task2_subclass=None):
    ...
    # 优先提取 Final Answer 部分
    final_answer_pattern = r'\*\*final answer:\*\*\s*(\w+)'
    final_answer_match = re.search(final_answer_pattern, pred_clean)
    
    if final_answer_match:
        final_answer = final_answer_match.group(1).strip().lower()
        # 直接使用Final Answer的结果
        if final_answer in ['legitimate', 'true', 'real']:
            return "legitimate"
        elif final_answer in ['fake', 'false']:
            return "fake"
    
    # 如果没有找到Final Answer，尝试从Self-Consistency Vote中提取
    vote_pattern = r'\*\*self-consistency vote:\*\*(.*?)(?:\*\*|$)'
    vote_match = re.search(vote_pattern, pred_clean)
    
    if vote_match:
        vote_text = vote_match.group(1).strip()
        # 从投票文本中提取最终决定
        if 'legitimate' in vote_text or 'true' in vote_text:
            return "legitimate"
        elif 'fake' in vote_text or 'false' in vote_text:
            return "fake"
    
    # 如果还是没有找到明确答案，尝试统计推理链中的结论
    reasoning_conclusions = []
    
    # 查找所有推理链的结论
    conclusion_patterns = [
        r'reasoning chain \d+:.*?conclusion:\s*(\w+)',
        r'chain \d+:.*?conclusion:\s*(\w+)',
        r'conclusion:\s*(\w+)'
    ]
    
    for pattern in conclusion_patterns:
        matches = re.findall(pattern, pred_clean)
        for match in matches:
            conclusion = match.strip().lower()
            if conclusion in ['legitimate', 'true', 'real', 'fake', 'false']:
                reasoning_conclusions.append(conclusion)
    
    if reasoning_conclusions:
        # 统计结论，采用多数投票
        legitimate_count = sum(1 for c in reasoning_conclusions if c in ['legitimate', 'true', 'real'])
        fake_count = sum(1 for c in reasoning_conclusions if c in ['fake', 'false'])
        
        if legitimate_count > fake_count:
            return "legitimate"
        elif fake_count > legitimate_count:
            return "fake"
    
    # 最后，如果所有方法都失败，回退到传统的关键词匹配
...
def calculate_binary_classification_metrics(data, task_type, task2_subclass=None):
    ...
    for item in data:
        ...
        # 从label字段的CoT-SC格式中提取真实标签
        label_text = str(item['label']).strip()
        
        # 使用正则表达式从label的Final Answer中提取真实标签
        final_answer_match = re.search(r'\*\*final answer:\*\*\s*(\w+)', label_text.lower())
        if final_answer_match:
            true_label = final_answer_match.group(1).strip().lower()
        else:
            # 如果没有找到Final Answer，跳过这个样本
            continue
        
        if task_type == "binary_classification":
            # 对于Task1和Task3的传统二分类
            if task2_subclass is None:
                if true_label in ['fake', 'false']:
                    true_labels.append("fake")
                elif true_label in ['legitimate', 'true']:
            ...
            # 对于Task2的二分类
                ...
                if "fake" in task2_subclass:
                    ...
                    if task2_subclass == "style_based_fake" and true_label == "style-based":
                        true_labels.append("legitimate")  # 这里legitimate表示正类
                    elif task2_subclass == "content_based_fake" and true_label == "content-based":
                        true_labels.append("legitimate")
                    elif task2_subclass == "integration_based_fake" and true_label == "integration-based":
                        true_labels.append("legitimate")
                    elif task2_subclass == "story_based_fake" and true_label == "story-based":
                        true_labels.append("legitimate")
                    ...
                    # 对于legitimate子类：style-based vs integration-based
                    if true_label == "style-based":
                        true_labels.append("legitimate")
                    elif true_label == "integration-based":
                        true_labels.append("fake")
                    else:
                        continue  # 跳过未知标签
...
def get_dataset_from_path(file_path):
    ...
    # CoT-SC 数据集识别
    if "cot_sc" in filename.lower() or "CoT_SC" in filename:
        if "_glm_" in filename:
            return "ChatGLM CoT-SC Dataset"
        elif "_llama3_" in filename or "_llama_" in filename:
            return "LLaMA CoT-SC Dataset"
        elif "_qwen" in filename.lower():
            return "Qwen CoT-SC Dataset"
        elif "_baichuan" in filename.lower():
            return "Baichuan CoT-SC Dataset"
        elif "_mistral" in filename.lower():
            return "Mistral CoT-SC Dataset"
        else:
            return "CoT-SC Dataset"
...
def find_result_files(base_dir="megafakeTasks"):
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for file in files:
            # 只查找包含cot_sc的jsonl文件，且在full目录下
            if file.endswith('.jsonl') and 'cot_sc' in file.lower() and 'full' in root:
...
def main():
    parser = argparse.ArgumentParser(description='分析CoT-SC模型预测结果')
    parser.add_argument('--dir', default='megafakeTasks', help='结果文件目录')
    parser.add_argument('--file', help='指定单个文件进行分析')
    parser.add_argument('--output', default='megafakeTasks/results/prediction_analysis_results_CoT_SC.csv', help='CSV输出文件名')
    parser.add_argument('--no-csv', action='store_true', help='不导出CSV文件')
    
    args = parser.parse_args()
    
    print("🚀 开始分析CoT-SC模型预测结果")
