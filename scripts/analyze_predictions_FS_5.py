...
该脚本专门用于分析Few-shot with 5 examples (FS-5)格式的模型预测结果。
FS-5格式特点：
- 使用5个示例进行Few-shot学习
- 预测结果格式简洁：Classification: fake/legitimate
- 标签格式包含推理过程，以fake/legitimate结尾

脚本会从predict字段的Classification部分提取预测结果，从label字段末尾提取真实标签。
"""
...
def normalize_prediction(pred_text, task_type, task2_subclass=None):
    ...
    # FS-5格式主要是"Classification: fake/legitimate"的形式
    # 优先提取Classification部分
    ...
    for pattern in classification_patterns:
        ...
        if match:
            ...
            # 检查分类结果
            if classification in ['legitimate', 'true', 'real']:
                return "legitimate"
            elif classification in ['fake', 'false']:
                return "fake"
    
    # 如果没有找到明确的Classification，检查文本末尾的关键词
    text_end = pred_clean[-50:]
    
    # 查找明确的结论词汇
    if any(word in text_end for word in ['fake', 'false']):
        return "fake"
    elif any(word in text_end for word in ['legitimate', 'true', 'real']):
        return "legitimate"
...
def calculate_binary_classification_metrics(data, task_type, task2_subclass=None):
    ...
    for item in data:
        ...
        # 从label字段的FS-5格式中提取真实标签
        label_text = str(item['label']).strip()
        
        # FS-5格式的标签以"legitimate"或"fake"结尾
        true_label = None
...
def get_dataset_from_path(file_path):
    ...
    # FS-5 数据集识别
    if "fs_5" in filename.lower() or "FS_5" in filename:
        if "_glm_" in filename:
            return "ChatGLM FS-5 Dataset"
        elif "_llama3_" in filename or "_llama_" in filename:
            return "LLaMA FS-5 Dataset"
        elif "_qwen" in filename.lower():
            return "Qwen FS-5 Dataset"
        elif "_baichuan" in filename.lower():
            return "Baichuan FS-5 Dataset"
        elif "_mistral" in filename.lower():
            return "Mistral FS-5 Dataset"
        else:
            return "FS-5 Dataset"
...
def find_result_files(base_dir="megafakeTasks"):
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for file in files:
            # 只查找包含fs_5的jsonl文件，且在full目录下
            if file.endswith('.jsonl') and 'fs_5' in file.lower() and 'full' in root:
...
def main():
    parser = argparse.ArgumentParser(description='分析FS-5模型预测结果')
    parser.add_argument('--dir', default='megafakeTasks', help='结果文件目录')
    parser.add_argument('--file', help='指定单个文件进行分析')
    parser.add_argument('--output', default='megafakeTasks/results/prediction_analysis_results_FS_5.csv', help='CSV输出文件名')
    parser.add_argument('--no-csv', action='store_true', help='不导出CSV文件')
    
    args = parser.parse_args()
    
    print("🚀 开始分析FS-5模型预测结果")
